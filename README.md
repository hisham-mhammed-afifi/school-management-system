...
A production-ready, multi-tenant SaaS backend for managing schools. Built with Node.js 24, TypeScript 5.8+, Express 5, Prisma 7, and PostgreSQL.
...
| Language | TypeScript 5.8+ (strict mode, ESM only) |
...
src/
  ...
  modules/         # 44 domain modules (routes -> controller -> service -> repository)
...
GET    /period-sets/:setId/time-slots
CRUD   /rooms
```
...
Use `requireFeature('feature_name', prisma)` middleware to gate routes.
...
- [ ] Review and restrict CORS origin to specific frontend domains
...
- [ ] Configure Redis for session management and caching (connection already in env)
...
### Email & Notifications
- [ ] Integrate email service (SendGrid, SES, etc.) for:
  - Password reset emails (currently logs token in dev mode)
  - Notification delivery (email/SMS channels)
...
- [ ] **Overdue invoice scanner**: Daily job to mark past-due invoices as `overdue`
- [ ] **Subscription expiry checker**: Daily job to notify/suspend expired school subscriptions
- [ ] Consider using `node-cron`, Bull/BullMQ, or external scheduler (CloudWatch Events, etc.)

### Performance
- [ ] Add Redis caching for frequently accessed data (permissions, school config)
- [ ] Add database indexes for common query patterns beyond what Prisma generates
- [ ] Consider read replicas for heavy reporting queries
- [ ] Tune `express-rate-limit` window and limit values for expected traffic

### Additional Features to Consider
- [ ] File upload support (student photos, documents) with cloud storage (S3, GCS)
- [ ] Webhook support for external integrations
- [ ] API versioning strategy for future breaking changes
- [ ] Pagination cursor-based option for large datasets
- [ ] Bulk import/export (CSV/Excel) for students, teachers, grades
- [ ] Report generation (PDF report cards, financial statements)
- [ ] Real-time features (WebSocket for live notifications)
- [ ] Multi-language support (i18n for error messages and notifications)
...
school-management/
  ...
  src/
    ...
    shared/
      errors/
        ...
        error-handler.ts    # Global error handler (Zod, Prisma, AppError)
      middleware/
  ...
  tests/
    ...
    integration/
      api.integration.test.ts  # Integration test suite (25+ tests)
```
